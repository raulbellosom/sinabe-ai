...
const { ensureCollection, ensurePayloadIndexes, upsertPoints, searchSimilar } = require('./src/qdrant');
...
app.post('/debug/ollama-embed', async (req, res) => {
  try {
    ...
    const [r] = await embedTextsRobust([text], { model, concurrency: 1, timeoutMs: REQ_TIMEOUT_MS });
...
app.post('/ingest', async (req, res) => {
  try {
    ...
    if (scope !== 'inventories') {
      return res.status(400).json({ error: 'scope no soportado (usa "inventories")', scope });
    }

    await ensureCollection(dim, collection);
    await ensurePayloadIndexes(collection);

    let offset = 0, totalIndexed = 0, totalSkipped = 0, page = 0;
    ...
    res.json({ ok: true, collection, dim, indexed: totalIndexed, skipped: totalSkipped, skippedSamples });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/search/hybrid', async (req, res) => {
  try {
    ...
    if (maybeSerial) {
      ...
      if (exactRows.length) {
        return res.json({ mode:'serial-exact', q, results: exactRows, suggestions: [] });
      }
    ...
    const [semRows, kwRows] = await Promise.all([
      require('./src/semantic').semanticSearchQdrant({ q: text || q, topK, collection, filters }),
    ...
    // 5) LLM rerank
    const reranked = await rerankWithLLM(q, fused, { topN: Math.min(fused.length, 12) });
    res.json({ mode: 'hybrid', q, parsed: { text, filters }, results: reranked.slice(0, topK) });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
app.post('/models/specs', async (req, res) => {
  try {
    ...
    const prompt =
\`Actúa como experto en hardware. Con base en conocimiento general (sin internet):
...
Aclara que puede variar por configuración y lote. Responde en español en formato de viñetas.\`;
    ...
    res.json({ id: row.id, brand: row.brandName, model: row.modelName, specs: text });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

app.listen(PORT, () => console.log(`AI API (hybrid) listening on port ${PORT}`));